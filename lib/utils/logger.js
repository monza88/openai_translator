...
exports.saveTranslationsLog = saveTranslationsLog;
...
function saveTranslationsLog(batchId, translations) {
    if (!DEBUG_LOG) {
        return; // 디버그 로그가 비활성화된 경우 함수 종료
    }
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-'); // ISO 8601 형식의 타임스탬프 생성
    ...
    try {
        fs_1.default.writeFileSync(path_1.default.join(logDir, fileName), JSON.stringify(translations, null, 2), 'utf8');
