...
router.post("/batch-translate", async (req, res) => {
    ...
    try {
        ...
        const testSheet = (0, googleSheet_1.getSheetData)("test", 1);
        console.log(`${sheetName} 테스트 :`, testSheet);
        let systemPrompt = "";
        if (promptFile) {
            try {
        ...
        const batches = Array.from({ length: totalBatches }, (_, i) => data.slice(i * BATCH_SIZE, (i + 1) * BATCH_SIZE));
        const translations = {};
        //각 배치 그룹에 대해 OpenAI로 번역 요청
        //여러 언어에 대해 번역 결과를 콜백 URL로 전송
        await Promise.all(batches.map(async (batchData, index) => {
...
async function mergeSheetData(sheetName, newTranslations) {
    const existngRows = await (0, googleSheet_1.getSheetData)(sheetName);
    // 2. key 기준 map
